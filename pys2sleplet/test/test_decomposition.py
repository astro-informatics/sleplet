...
def test_decompose_all_polar(slepian_polar_cap, earth_polar_cap) -> None:
    ...
    """
    f_p = slepian_forward(
        L, earth_polar_cap.multipole, slepian_polar_cap, method="integrate_region"
    )
    g_p = slepian_forward(
        L, earth_polar_cap.multipole, slepian_polar_cap, method="integrate_sphere"
    )
    h_p = slepian_forward(
        L, earth_polar_cap.multipole, slepian_polar_cap, method="harmonic_sum"
    )
...
def test_decompose_all_lim_lat_lon(slepian_lim_lat_lon, earth_lim_lat_lon) -> None:
    ...
    """
    f_p = slepian_forward(
        L, earth_lim_lat_lon.multipole, slepian_lim_lat_lon, method="integrate_region"
    )
    g_p = slepian_forward(
        L, earth_lim_lat_lon.multipole, slepian_lim_lat_lon, method="integrate_sphere"
    )
    h_p = slepian_forward(
        L, earth_lim_lat_lon.multipole, slepian_lim_lat_lon, method="harmonic_sum"
    )
...
def test_equality_to_harmonic_transform_polar(
    ...
    """
    f_p = slepian_forward(L, earth_polar_cap.multipole, slepian_polar_cap)
    f_slepian = slepian_inverse(L, f_p, slepian_polar_cap)
    f_harmonic = ssht.inverse(earth_polar_cap.multipole, L)
...
def test_equality_to_harmonic_transform_lim_lat_lon(
    ...
    tests that fp*Sp up to N is roughly equal to flm*Ylm
    """
    f_p = slepian_forward(L, earth_lim_lat_lon.multipole, slepian_lim_lat_lon)
    f_slepian = slepian_inverse(L, f_p, slepian_lim_lat_lon)
    f_harmonic = ssht.inverse(earth_lim_lat_lon.multipole, L)
...
def test_pass_rank_higher_than_available(slepian_polar_cap, earth_polar_cap) -> None:
    ...
    """
    sd = SlepianDecomposition(L, earth_polar_cap.multipole, slepian_polar_cap)
