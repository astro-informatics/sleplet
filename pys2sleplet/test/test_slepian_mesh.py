...
def test_decompose_all_mesh(slepian_mesh, mesh_field_region) -> None:
    ...
    """
    field = mesh_inverse(slepian_mesh.mesh, mesh_field_region.coefficients)
...
def test_forward_inverse_transform_slepian(slepian_mesh, mesh_field_region) -> None:
    ...
    f_slepian = slepian_mesh_inverse(slepian_mesh, f_p)
    f_harmonic = mesh_inverse(slepian_mesh.mesh, mesh_field_region.coefficients)
    assert_allclose(
        np.abs(f_slepian - f_harmonic)[slepian_mesh.mesh.region].mean(),
        0,
        atol=0.03,
    )
